...
const AdminDashboard = () => {
  ...
  const resendTicket = async (ticketId) => {
    ...
    try {
      ...
      if (res.ok) alert('Ticket sent successfully!');
    } catch (err) {
      console.error(err);
    }
  };
          ...
          <nav className="space-y-4 flex-grow">
            {[
              ...
              <button
                ...
                className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl font-bold transition-all group ${activeTab === tab.id
                    ? 'bg-primary text-white shadow-xl shadow-primary/20'
                    : 'hover:bg-white/5 hover:text-white'
                  }`}
              >
                <tab.icon className={`h-5 w-5 transition-transform group-hover:scale-110 ${activeTab === tab.id ? 'text-white' : 'text-slate-500 group-hover:text-primary'}`} />
              ...
              <form onSubmit={handleFlightSubmit} className="p-8 grid grid-cols-2 gap-6 max-h-[70vh] overflow-y-auto">
                <div className="col-span-2 grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input
                      required
                      type="text" value={flightForm.airline} onChange={e => setFlightForm({ ...flightForm, airline: e.target.value })}
                  ...
                  <div className="space-y-2">
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Flight Number</label>
                    <input
                      required
                      type="text" value={flightForm.flight_number} onChange={e => setFlightForm({ ...flightForm, flight_number: e.target.value })}
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-4 focus:ring-2 focus:ring-primary outline-none font-bold appearance-none"
                  >
                    <option value="">Select Airport</option>
                    {data.airports.map(a => (
                      <option key={a.id} value={a.id}>{a.city} ({a.iata_code})</option>
                    ))}
                  </select>
                </div>
                ...
                <div className="col-span-2 grid grid-cols-2 gap-6 pt-4 border-t border-slate-100 mt-4">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-slate-500 ml-4 tracking-widest">Economy Price ($)</label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-slate-500 ml-4 tracking-widest">Total Seats</label>
                    <input
                      required
                      type="number"
